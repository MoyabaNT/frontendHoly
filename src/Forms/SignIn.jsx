...
const SignIn = () => {
  ...
  const handleSignIn = async (e) => {
    ...
    try {
      ...
      localStorage.setItem('idToken', token); // Store token for API requests
  ...
  return (
    <div className="min-h-screen bg-[#4A2C2A] font-sans flex items-center justify-center">
      <div className="fixed top-1/2 left-1/2 bg-[#5E3A36] p-12 rounded-lg z-[2] -translate-x-1/2 -translate-y-1/2 font-light text-white animate-[hide_3s_both] [animation-timeline:scroll()] [animation-range:0%_10%]">
        ...
        <form className="mt-8 w-80 space-y-6" onSubmit={handleSignIn}>
          <div>
          ...
          {error === 'Please verify your email before signing in.' && (
            <button
              ...
              className="w-full mt-2 py-2 px-4 bg-transparent border border-white/20 text-white rounded-md hover:bg-white/10 transition-colors duration-300"
            >
